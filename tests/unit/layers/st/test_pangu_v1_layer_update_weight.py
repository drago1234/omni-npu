...
def _snapshot_parameters_from_model(
    model: pangu_ultra_moe_mod.PanguUltraMoEForCausalLM,
...
def test_high_player_update_weight(monkeypatch):
    monkeypatch.setattr(
    ...
    with set_current_vllm_config(worker.vllm_config):
        with allocator.use_memory_pool(tag="weights"):
            # Path-1: initial load via model script + vLLM post process.
