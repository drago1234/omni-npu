...
class OpenPanguMultiTokenPredictor(nn.Module):
    ...
    def compute_logits(
        ...
        logits = self.logits_processor(
            mtp_layer.shared_head.head, mtp_layer.shared_head(hidden_states).float() if model_extra_config.dtype != torch.float16 else mtp_layer.shared_head(hidden_states)
        )
        return logits
