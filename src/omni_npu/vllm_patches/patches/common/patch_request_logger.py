...
"""Backport request/completion logging patches for release branches."""
...
_ORIGINAL_INIT_APP_STATE = openai_api_server.init_app_state
...
@register_patch("OpenAICompletionInitAppStateLogOutputsPatch", openai_api_server)
class OpenAICompletionInitAppStateLogOutputsPatch(VLLMPatch):
    _attr_names_to_apply = ["init_app_state"]
    ...
    async def init_app_state(
        ...
        await _ORIGINAL_INIT_APP_STATE(engine_client, state, args)
        serving_completion = getattr(state, "openai_serving_completion", None)
        if serving_completion is not None:
            serving_completion.enable_log_outputs = args.enable_log_outputs
