...
def _get_thinking_token_budget(params: SamplingParams | None) -> int | None:
    if params is None or params.extra_args is None:
        return None
    return params.extra_args.get(_THINKING_TOKEN_BUDGET_KEY)
...
def _set_thinking_token_budget(
    ...
    if params.extra_args is None:
        params.extra_args = {}
    params.extra_args[_THINKING_TOKEN_BUDGET_KEY] = thinking_token_budget
    return params
...
def _get_request_thinking_token_budget(
    ...
    if model_extra:
        return model_extra.get(_THINKING_TOKEN_BUDGET_KEY)
    return None
...
class SamplingParamsPatch(VLLMPatch):
    ...
    @staticmethod
    def from_optional(
            thinking_token_budget: int | None = 131072,
            **kwargs,
    ) -> SamplingParams:
...
class InputBatchPatch(VLLMPatch):
    ...
    def _make_sampling_metadata(self):
        ...
        if not self.no_thinking_budget:
...
class SamplerPatch(VLLMPatch):
    ...
    def apply_logits_processors(
        ...
        needs_thinking = holder is not None and holder.has_tracked_requests() and holder.is_enabled

        output_token_ids = sampling_metadata.output_token_ids
        if predict_bonus_token and (any_penalties_or_bad_words or needs_thinking):
...
class SamplingMetadataPatch(VLLMPatch):
    ...
    def apply(cls):
        target = cls._target
        name = "thinking_budget_state_holder"
