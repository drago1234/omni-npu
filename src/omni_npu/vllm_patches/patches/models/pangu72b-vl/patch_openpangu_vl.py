...
from vllm.multimodal.inputs import MultiModalKwargs, MultiModalFeatureSpec
...
dynamic_module = types.ModuleType("openpangu_72b_vl")
sys.modules[models.__name__+".openpangu_72b_vl"] = dynamic_module
models.openpangu_72b_vl = dynamic_module
...
class NPURMSNorm(nn.Module):
    ...
    def forward(self, x, residual=None, quant_symbol=False):
...
def forward_W8A8Dynamic_FusedMLP(layer,x):
    def _apply_W8A8DynamicFusedMLPMethod(layer, x: torch.Tensor, x_scale: torch.Tensor, original_dtype):
        ...
        int_int32, int_scale = torch_npu.npu_dequant_swiglu_quant(
            y_int32,
            weight_scale=layer.gate_up_proj.weight_scale.float(),
        ...
        bias = None if (layer.down_proj.tp_rank > 0) else layer.down_proj.bias
    ...
    x = _apply_W8A8DynamicFusedMLPMethod(layer, x, x_scale, original_dtype)
...
class OpenPanguVisionAttention(nn.Module):
    ...
    def forward(
        ...
        if self.align_train:
            ...
            q, k, v = (rearrange(x, "s (b n d) -> s b n d", d=self.hidden_size_per_attention_head, b=1).contiguous()
                   for x in (q, k, v))
            q = npu_rotary_position_embedding(q, cos, sin, 0)
            k = npu_rotary_position_embedding(k, cos, sin, 0)
            q, k, v = (rearrange(x, "s b n d -> b s n d", d=self.hidden_size_per_attention_head, b=1)
                   for x in (q, k, v))
        else:
        ...
        attn_out = torch_npu.npu_fusion_attention(
            q, k, v, self.num_attention_heads_per_partition, 'TND',
            pse=None,
            padding_mask=None,
            atten_mask=None,
            ...
            inner_precise=0,
            sparse_mode=0,
            actual_seq_qlen=[0, *cu_seqlens.cumsum(0).tolist()],
            actual_seq_kvlen=[0, *cu_seqlens.cumsum(0).tolist()],
            )[0]
...
class OpenPanguVisionMLP(nn.Module):
    ...
    def __init__(self,
        ...
        if quant_config is not None:
            PAD_UNIT=128
            if hidden_features % PAD_UNIT != 0:
                hidden_features = (hidden_features + PAD_UNIT - 1) // PAD_UNIT * PAD_UNIT #3420 -> 3456, 6840 -> 6912
    ...
    def forward(self, x: torch.Tensor):
        if self.hidden_act == "silu":
            if False:
                return forward_W8A8Dynamic_FusedMLP(self,x)
            else:
                x, bias = self.gate_up_proj(x)
                if bias is not None:
                    x = x + bias
                x = torch_npu.npu_swiglu(x)
        else:
            x, bias = self.up_proj(x)
            if bias is not None:
                x = x + bias
            x = self.act_fn(x)
        x, bias = self.down_proj(x)
        if bias is not None:
            x = x + bias
        return x
...
class OpenPanguVisionRotaryEmbedding(nn.Module):
    ...
    def __init__(self, dim: int, theta: float = 10000.0) -> None:
        ...
        self.inv_freq = self.inv_freq.to(torch.bfloat16)

    def update_freqs_cache(self, seqlen: int) -> None:
        if seqlen > self._seq_len_cached:
            self._seq_len_cached = seqlen * 2
            seq = torch.arange(self._seq_len_cached, device=self.inv_freq.device, dtype=self.inv_freq.dtype)
...
class OpenPanguVisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        with parallel_state.patch_tensor_parallel_group(self._tp_group):
            ...
            norm_layer = partial(NPURMSNorm, eps=norm_eps)
            ...
            self.quant_symbol = True if quant_config else None
            ...
            self.final_layernorm = NPURMSNorm(self.hidden_size, eps=norm_eps)
    ...
    def load_weights(self, weights) -> set[str]:
        def _padding_weight(name: str, w: torch.Tensor,quant_symbol: bool) -> torch.Tensor:
            if "gate_proj" in name or "up_proj" in name:
                dim, size = 0, w.size(0)
            elif "down_proj" in name:
                dim, size = 1, w.size(-1)
                if quant_symbol and (name.endswith("weight_scale") or name.endswith("weight_offset")):
                    return w
            else:
                return w
            PAD_UNIT = 128 if quant_symbol else self.tp_size
            pad_len = -size % PAD_UNIT
            if pad_len == 0:
                return w
            pad = [0] * (w.ndim * 2)
        ...
        with parallel_state.patch_tensor_parallel_group(self._tp_group):
            ...
            for name, loaded_weight in weights:
                if self.hidden_act == "silu":
                    loaded_weight = _padding_weight(name, loaded_weight, self.quant_symbol)
...
class openpangu_vlPatch(VLLMPatch):
    ...
    class OpenPanguVLForConditionalGeneration(nn.Module, SupportsMultiModal,
        ...
        def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
            ...
            text_model_architectures = self.config.text_config.model_architectures if "model_architectures" in \
                self.config.text_config else "PanguEmbeddedForCausalLM"
            self.language_model = init_vllm_registered_model(
                vllm_config=vllm_config,
                prefix=maybe_prefix("openpangu", "language_model"),
                architectures=[text_model_architectures],
            )
        ...
        def _process_image_input(self, image_input) -> tuple[torch.Tensor, ...]:
            ...
            if image_input["type"] == "image_embeds":
                ...
                image_use_fast = self.vllm_config.model_config.hf_image_processor_config.get("image_use_fast", True)
    ...
    ModelRegistry.register_model(
        ...
        "vllm.model_executor.models.openpangu_72b_vl:OpenPanguVLForConditionalGeneration",
    )
    ModelRegistry.register_model(
        "PanguProMoEV2ForCausalLM",
        "vllm.model_executor.models.openpangu:PanguProMoEV2ForCausalLM",
    )
    ModelRegistry.register_model(
        "OpenPanguVLMTPModel",
        "vllm.model_executor.models.openpangu_vl_mtp:OpenPanguVLMTP",
    )
