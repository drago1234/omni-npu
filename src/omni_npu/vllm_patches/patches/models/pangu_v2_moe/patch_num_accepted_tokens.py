...
# for pangu_v2_moe NPUMLAMetadataBuilder spec-decode support
def _build_attention_metadata(
    ...
    def _build_attn_group_metadata(
        ...
        #### patch starts
        if use_spec_decode and isinstance(builder, MLACommonMetadataBuilder):
            common_attn_metadata.num_accepted_tokens = self.num_accepted_tokens.gpu[:num_reqs_padded]
        #### patch ends
...
@register_patch("PanguV2MoeGPUModelRunnerPatch", GPUModelRunner)
class PanguV2MoeGPUModelRunnerPatch(VLLMPatch):
    """
    Patch for GPUModelRunner to support NPUMLAMetadataBuilder and NPUDSAMetadataBuilder
    in speculative decoding for pangu_v2_moe.

    This patch modifies the _build_attention_metadata method to add support for
    NPU-specific metadata builders in speculative decoding.
    """

    _attr_names_to_apply = ["_build_attention_metadata"]

    ##### patch start
    _build_attention_metadata = _build_attention_metadata
    ##### patch end


M = TypeVar("M", bound=MLACommonMetadata)
_original_build = MLACommonMetadataBuilder.build
@register_patch("PanguV2MoeMLACommonMetadataBuilderPatch", MLACommonMetadataBuilder)
class PanguV2MoeMLACommonMetadataBuilderPatch(VLLMPatch):
    _attr_names_to_apply = ["build"]
    ...
    def build(
        ...
        if hasattr(common_attn_metadata, "num_accepted_tokens") and attn_metadata.decode is not None:
            attn_metadata.decode.num_accepted_tokens = common_attn_metadata.num_accepted_tokens
