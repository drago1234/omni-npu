...
class NPUFusedMoEPermuteExpertsUnpermute(FusedMoEPermuteExpertsUnpermute):
    def __init__(
        ...
        if self.quant_config.use_int8_w8a8:
            ...
            self.scale_2 = torch.ones(layer.w13_weight_int4_scale.shape, dtype=torch.float32, device="npu").view(-1, layer.w13_weight_int4_scale.shape[-1])
        else:
            self.scale_2 = None
    ...
    def apply(
        ...
        if self.quant_config.use_int8_w8a8:
            ...
            gate_up_proj = torch_npu.npu_grouped_matmul(
                [hidden_states], 
                [w1], 
                scale=[self.quant_config.w1_scale],
            ...
            pertoken_scale = torch.ones(a1q_scale.shape, dtype=torch.float32, device="npu")
            gate_up_proj, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
                gate_up_proj,
                weight_scale=self.scale_2,
            ...
            hidden_states_experts = torch_npu.npu_grouped_matmul(
                [gate_up_proj], 
                [w2], 
                scale=[self.quant_config.w2_scale],
