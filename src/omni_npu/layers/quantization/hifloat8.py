...
class Hifloat8MoEMethod(FusedMoEMethodBase, NPUFusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        layer.w2_weight_scale = torch.nn.Parameter(layer.w2_weight_scale.to(torch.float32).squeeze(-1), requires_grad=False)
    ...
    def apply_experts(
        ...
        intermediate_hidden_states, pertoken_scale = torch_npu.npu_dequant_swiglu_quant(
            gate_up_proj,
            weight_scale=None, activation_scale=None, bias=None,
            quant_offset=None, quant_scale=quant_scale,
            group_index=expert_tokens,
            activate_left=True, quant_mode=1,
            dst_type=torch_npu.hifloat8, round_mode=1,
        )
