...
class NPUmHCModule(mHCModule):
    ...
    def hc_pre(self, x: torch.Tensor):
        if self.merge_layer_only_pre:
            ...
            self.branch_alpha = torch.cat([self.branch_alpha_pre, self.branch_alpha_post, self.branch_alpha_res])
            self.branch_beta = torch.cat([self.branch_beta_pre, self.branch_beta_post, self.branch_beta_res])
            y, h_post, h_res, _, _, _ = torch.ops.custom.npu_manifold_constrained_hyper_connection_pre(x.view(-1, self.num_stream, self.hidden_size), self.phi.weight,
                    self.branch_alpha, self.branch_beta, gamma=self.norm_gamma.view(self.num_stream, -1), norm_eps=self.norm_eps, hc_eps=self.hc_eps)
